...
- **批次上下文感知**：你收到的是同一批次中的所有文件列表。请利用整体上下文来辅助判断：
  - 如果某个文件是批次中唯一一个没有集数编号（如 [01], EP01）的文件，而其余文件都带有连续编号 → 该文件极可能是电影/剧场版(movie)
  - 如果一个批次里只有一个单独的视频文件，且没有集数编号 → 极大概率是电影(movie)
  - 观察文件名中的来源字幕组/压制组是否相同，来判断它们是否属于同一部作品
